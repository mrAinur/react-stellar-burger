...
export default function IngredientDetails() {
  ...
  const info = useSelector(state => state.ingredients.ingredients).find(
    item => item._id === id.ingredientId,
  );
  ...
  return info ? (
    <div className={background !== null ? style.main : style.mainTop}>
      <div
        className={`${
          background !== null ? style.head : style.headCenter
        } mt-10`}
      >
      ...
      <img
        ...
        alt="Картинка ингредиента бургера"
      />
      <p className={`${style.info} text text_type_main-medium mt-4`}>
        {info.name}
      </p>
