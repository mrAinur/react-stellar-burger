...
export default function Feed() {
  ...
  const getOrderInfo = items => {
    return items.map(item => {
      return <Link to={`/feed/${item.number}`} state={{ background: location }} className={style.link} key={item._id}><OrderInfoRectangle data={item} element={"public"}/></Link>
    })
  }
